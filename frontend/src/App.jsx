...
// import { PieChart, Pie, Cell, Tooltip as RechartsTooltip, ResponsiveContainer, ComposedChart, Line, Bar, XAxis, YAxis, CartesianGrid, Legend } from 'recharts'
// import { 
...
//   // --- UPGRADE C: Variance Sorting & Filtering ---
...
//   const mdrVolume = useMemo(() => results ? results.exceptions.filter(e => e.type === 'MDR_FEE').reduce((acc, e) => acc + e.variance, 0) : 0, [results])
//   const dropoutVolume = useMemo(() => results ? results.exceptions.filter(e => e.type === 'DROPOUT').reduce((acc, e) => acc + e.variance, 0) : 0, [results])
...
//               <span style={{ fontSize: '0.75rem', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Total Ledger Volume</span>
//               <span style={{ fontSize: '1.8rem', fontWeight: 700, margin: '0.2rem 0', color: t.text }}>₹{results.summary.total_ledger_volume.toLocaleString()}</span>
//               <span style={{ fontSize: '0.75rem', color: t.muted }}>Gross Expected Value</span>
//             </div>
            
//             <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
//               <span style={{ fontSize: '0.75rem', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Reconciliation Rate</span>
//               <span style={{ fontSize: '1.8rem', fontWeight: 700, margin: '0.2rem 0', color: '#10b981' }}>{results.summary.reconciliation_rate}%</span>
//               <span style={{ fontSize: '0.75rem', color: t.muted }}>{results.summary.reconciled_count} balanced records</span>
//             </div>

//             <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
//               <span style={{ fontSize: '0.75rem', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Expected MDR Fees</span>
//               <span style={{ fontSize: '1.8rem', fontWeight: 700, margin: '0.2rem 0', color: '#3b82f6' }}>₹{mdrVolume.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
//               <span style={{ fontSize: '0.75rem', color: t.muted }}>Standard Gateway Deductions</span>
//             </div>

//             <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
//               <span style={{ fontSize: '0.75rem', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>Critical Dropouts</span>
//               <span style={{ fontSize: '1.8rem', fontWeight: 700, margin: '0.2rem 0', color: '#ef4444' }}>₹{dropoutVolume.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
//               <span style={{ fontSize: '0.75rem', color: t.muted }}>Unaccounted Revenue Slippage</span>
//             </div>
//           </div>

//           {/* TIER 2: Dual Chart Row */}
//           <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1.5rem', marginBottom: '2rem' }}>
//             <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
//               <h3 style={{ margin: '0 0 1rem 0', fontSize: '0.9rem', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Composition Breakdown</h3>
...
//             <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
//               <h3 style={{ margin: '0 0 1rem 0', fontSize: '0.9rem', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Top Exceptions: Expected vs. Variance Drift</h3>
//               <div style={{ height: '220px', width: '100%' }}>
//                 <ResponsiveContainer width="100%" height="100%">
//                   <ComposedChart data={dualAxisData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
//                     <CartesianGrid strokeDasharray="3 3" stroke={t.border} vertical={false} />
//                     <XAxis dataKey="name" stroke={t.muted} fontSize={11} tickLine={false} axisLine={false} />
//                     <YAxis yAxisId="left" stroke={t.muted} fontSize={11} tickLine={false} axisLine={false} tickFormatter={(val) => `₹${val/1000}k`} />
//                     <YAxis yAxisId="right" orientation="right" stroke="#ef4444" fontSize={11} tickLine={false} axisLine={false} />
//                     <RechartsTooltip contentStyle={{ backgroundColor: '#1f2937', border: 'none', borderRadius: '8px', color: '#fff' }} />
//                     <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '10px' }} />
//                     <Bar yAxisId="left" dataKey="expected" name="Gross Amount (₹)" fill="#3b82f6" radius={[4, 4, 0, 0]} barSize={24} />
//                     <Line yAxisId="right" type="monotone" dataKey="variance" name="Lost to Variance (₹)" stroke="#ef4444" strokeWidth={3} dot={{ r: 4, fill: '#ef4444' }} activeDot={{ r: 6 }} />
//                   </ComposedChart>
//                 </ResponsiveContainer>
//               </div>
//             </div>
...
//           {/* TIER 3: The Data Table (Restored!) */}
      ...
      {mainView === 'results' && results && (
        <div style={{ animation: 'fadeIn 0.5s ease' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1.5rem', marginBottom: '2rem' }}>
            ...
            {/* Donut Chart with Structured Side-by-Side Legend */}
            <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}`, borderRadius: '10px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
              <h3 style={{ margin: '0 0 1rem 0', fontSize: '0.9rem', color: t.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Composition Breakdown</h3>
              ...
              <div style={{ display: 'flex', alignItems: 'center', height: '220px', width: '100%' }}>
                ...
                <div style={{ width: '50%', display: 'flex', flexDirection: 'column', gap: '0.85rem', paddingLeft: '1rem' }}>
                  {chartData.map((item, idx) => {
                    const total = results.summary.total_records || 1
                    ...
                    return (
                      <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          <span style={{ fontSize: '0.85rem', color: t.text, fontWeight: 500 }}>{item.name}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <span style={{ fontSize: '0.85rem', fontWeight: 700, color: t.text }}>{item.value}</span>
                          <span style={{ fontSize: '0.75rem', color: t.muted }}>({pct}%)</span>
                        </div>
                      </div>
                    )
                  })}
